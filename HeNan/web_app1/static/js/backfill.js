...
async function startScanBackfillTask() {
    const startBtn = document.getElementById('backfill-start-btn');
    const progressWrap = document.getElementById('backfill-progress');
    const statusEl = document.getElementById('backfill-status-text');
    ...
    try {
        ...
        const fields = collectBackfillFields();
        ...
        const response = await fetch(`${API_BASE}/copyright/backfill/scan-fields/start`, {
            ...
            body: JSON.stringify({ media_names: mediaNames, fields })
        });
        const result = await response.json();
        if (result.code !== 200) {
        ...
        startBtn.innerHTML = '回填执行中...';
        showToast(`回填任务已启动，剧名数：${result.data.total_media}`, 'success');
        ...
        try {
            ...
            if (data.status === 'completed' || data.status === 'failed') {
                ...
                if (data.status === 'completed') {
                    showToast(`回填完成：更新${data.updated_episodes || 0}，未命中${data.missed_episodes || 0}`, 'success', 5000);
                } else {
